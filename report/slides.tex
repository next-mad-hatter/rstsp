...
\begin{slide}[toc=]{Strongly Balanced Tours: Construction}
  ...
  \item
    Strongly balanced tours:
    \begin{equation}
      \tau \in \mathcal{T}_n : \left\{\tau\right\} \in \mathcal{B}_n,
    \end{equation}
...
\begin{slide}[toc=]{Strongly Balanced Tours: Dynamic Programming Solution}
  ...
  \item
    ...
    \item
      ...
      \begin{quote}
      {\itshape
        a minimum cost set of paths belonging to a single element of $\mathcal{B}_n$
        through cities larger than $m$ (and having end points in given intervals)
        which, per path concatenation, complements said
        intervals to a single cycle.
      }
      \end{quote}
      \vspace{2mm}
    \end{itemize}
