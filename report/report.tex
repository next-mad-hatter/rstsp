...
Given the set $\mathcal{N}_n$ of $n$ cities, let $\mathcal{S}_n$ denote the symmetric group (on $\mathcal{N}_n$).
...
\begin{proof}
  ...
  produces a linear order on paths in $V\left(\mathcal{N}_n^2\right)$ (bar
  $(n,n)$), and hence also on $\operatorname{Pyr}_n$.
...
\null\hfill%
\subfloat[25 TSPLIB instances: observed tour quality]{%
  \includegraphics[width=0.67\textwidth]{../plot/build/tsplib_val.pdf}
}\hfill\null\\
\null\hfill%
\subfloat[25 TSPLIB instances: runtime measured]{%
...
is general enough to be easily extended to other solutions and
efficient enough for study of such solutions or combinations\footnote{%
  Also via the standalone executable, which
  provides the option of choosing the starting tour
  (which is automatically transformed to given neighbourhood's centre).
}
