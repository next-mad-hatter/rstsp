...
\begin{example}
  ...
  \begin{tabular}{gcgrg}
    ...
    \bottomrule
  \end{tabular}
  \caption{Sample path taken in the strongly balanced recursion tree ($M \geq 2$)
    along with correspondingly computed optimum. % (for some fixed sample cost matrix).
...
\caption{Computed maximum number of node types for values of node size limit $M$ up to $6$,
  along with the number of cities $m$ after which fixed point was reached.}%
\label{tbl:nodeTypes}%
\end{table}
...
\needspace{5\baselineskip}
\mylisting[
  caption={[Sample SML Session]Sample SML Session.},
