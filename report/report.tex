...
\begin{remark}
  Strongly balanced tours, per \cref{def:sbtour},
...
\label{prop:sball}
  ...
  while also complementing some element of $\mathcal{B}_n$ to a strongly balanced tour.
  Then
  \[
...
This defines a recurrence relation to which we can again apply memoized
traversing technique seen in pyramidal case (in fact, apart from
description of recursion tree, counting tour reconstruction rules, our
implementation uses same code for both) and compute a minimum total cost
element in $W\left(m,N\right)$ for each node, and thus a minimum cost tour
in $\operatorname{SBal}_n^M$ --- sample trace of such computation can be
seen in \cref{fig:sbtrace} ---
where the tour reconstruction rules can be deduced directly from given
relation and are left as exercise to the reader (they amount to inserting
into the solution, with possible reversal and joining, paths $(m,a_1)$ or $(a_1,m,a_2)$).
...
\begin{example}
  ...
  along with the strongly balanced tour corresponding to the taken path
  and sample optimum solution from $W(m,N)$ as it is constructed
  while returning along the path.
  ...
  \begin{tabular}{gcgrg}
    ...
    \bottomrule
  \end{tabular}
  \caption{Sample path taken in the strongly balanced recursion tree along with optimum (for some sample cost matrix) solution for each node.  Elements of $\mathcal{B}_m$ corresponding to the nodes are given as illustration.}
  \label{tbl:sbpath}
  \end{table}
\end{example}
...
\begin{enumerate}[(i)]
  \item
    ...
    but can reasonably be assumed to contribute to time complexity
    a polynomial factor only
    (note that we only need to reconstruct the optimum tour);
  \item
    ad memory access:
    while enumeration of {\defstyle node types} (introduced below) produces
    a polynomial time collision-free solution to this,
    in practice we were able to achieve better results with polynomial hash function
...
here grows in size only linearly with $n$ --- which, lifting out the tour
reconstruction step, would make time complexity of our solution to strongly
balanced TSP linear.
...
for values of $M$ up to $6$, which are presented in \cref{tbl:nodeTypes}
...
In our tests we have not been able to improve on
\[
  \mathcal{F}\left(
  \left<\left( 1\;2\;\cdots\;n\right)\right>
    %\begin{pmatrix}
