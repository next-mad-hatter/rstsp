...
\begin{remark}
\annotation{Add an example for concatenation, setup precedence over union.}
...
\label{rem:order}
  ...
  as it propagates to any construct using $\operatorname{tsp}$ ---
  such as iterative methods presented in next section.
\end{remark}
...
computing, in place of a function over some set, said function's graph over
the set.
In what now follows, $X$ can be seen as a memory holding evaluated results.

\begin{proposition}[memoized version of $\Phi_C$]
\label{prop:pyrmem}
\annotation{better layout via alignedat?}
...
produced \cref{fig:pyrtrace} as its execution trace, closely mirrors this exact
definition.
It is also a reasonably efficient implementation:
evaluating $\widehat{\Phi}_C$ following its definition corresponds to
depth first search over the recursion tree, which is quadratic in size
...
is visited no more than once, we intuitively arrive at following:
...
\begin{proof}
  ...
  At this point we would like to make several assumptions which we shall address later:
  \begin{enumerate}[(i)]
    \item memory access (search for $v^\ast$ in first case of \eqref{eq:pyrmem}, and set union in the other two)
      can be accomplished in constant time;
    \item $\argmin \omega_C$ in said definition is computable in constant time;
    \item tour concatenation in said definition is computable in constant time.
  \end{enumerate}
  If, for a bound $M$,
...
Referential transparency, space complexity\ldots
