...
\begin{remark}
  ...
  explicitly (as end points) allows us to employ same notion of cost for
...
\begin{remark}
Whenever the sets $M,T$ from above definition are established elsewhere,
...
\begin{remark}
  We deliberately omit the choice of $\tau^\ast$ from notation of induced
...
Dynamic programming is a fruitful approach to solving many problems in
...
\end{define}
We note here that the set $\operatorname{Pyr}_n$, considered neighbourhood of
$\tau^\ast = \left(1, 2, \ldots, n, 1\right)$, induces a neighbourhood in $\mathcal{T}_n$.
...
This definition of pyramidal tours, while explaining the name,
does not offer an obvious solution to the optimization problem.
To achieve that, first we observe that every pyramidal tour
...
This argument, while also showing that $\left|\operatorname{Pyr}_n\right| =
\Theta\left(2^n\right)$, makes following construction obvious.
...
\end{proposition}
\begin{proof}
  The second case in \cref{eq:allpyr} becomes apparent when,
...
\begin{proposition}[dynamic programming solution to pyramidal TSP]
\label{prop:pyrdp}
  Given a cost matrix $C \in \mathbb{R}^{n \times n}$
  and a linear order $L$ on $\mathcal{P}_n$, define,
  for cities $i,j$ in $\mathcal{N}_n$,
  $\Phi_{L,C}\left(i,j\right)$ to be the minimum (according to $L$)
  lowest cost path in $V\left(i,j\right)$
  from \cref{prop:allpyr}.
  Then for some such order $L$ both
  %FIXME: limits in min, argmin?
  \begin{align*}
    \begin{split}
    \Phi_{L,C}\left(i,j\right) & =
    \begin{cases}
      \left(i,j\right), & n \in \left\{i,j\right\}, \\
      \min_L \argmin_T \omega_C & \text{otherwise},
    \end{cases}
    \\
    \text{with}\;
      T & =
      \left\{
      \left(i\right) \oplus \Phi_{L,C}\left(k,j\right),
      \Phi_{L,C}\left(i,k\right) \oplus \left(j\right)
      \right\},
      k \; \text{as above}.
    \end{split}
  \end{align*}
  and
  $
    \Phi_{L,C}\left(1,1\right) = \operatorname{TSP_{L,\mathbb{R}^{n \times n},\operatorname{Pyr}_n}}\left(C\right)
  $
  both hold.
\end{proposition}
