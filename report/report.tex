...
\begin{abstract}
  ...
  optimization, and the traveling salesman problem as well as its special
  cases --- where dynamic programming can often be applied effectively ---
  are no exception.

  Here we want to describe the general procedure of constructing, given a
  ...
  the TSP of which the first is widely known, albeit not its use as a
  heuristic method, and the second had yet at all to see a working
  implementation --- which could be seen as main incentive behind this
  project --- and present results thus obtained.
...
\label{rem:order}
  ...
  such as iterative methods we shall present here.
\end{remark}
...
As in preceding section, the solutions as they are presented herein
can be taken to reflect inner workings of our code.
...
a number of polynomially testable classes which posess pyramidal solutions exists \cite{baki}.
\end{remark}
...
and thereby a minimum cost tour in $\operatorname{SBal}_n^M$.  Sample trace of
such computation can be seen in \cref{fig:sbtrace}.  Exact tour
reconstruction rules can be deduced directly from given relation and are
...
We can recognize following crucial points:
\begin{itemize}
  \item
...
the constant factor $K$ does seem intimidating, reflecting the runtime
...
which remains to be seen as an open question ---
...
convenience and, while being not too low level language, can still offer
...
\begin{itemize}
  \item as standalone executable
    (compiled via {\ttfamily MLton}, {\ttfamily MLton/MinGW} or {\ttfamily Poly/ML});
  ...
  \item via shared library interface
    (builds via {\ttfamily MLton} or {\ttfamily MLton/MinGW})
    as shown in \autoref{app:so}.
\end{itemize}

All code written for this project
(along with precompiled Windows/{\ttfamily x86} build)
...
\begin{itemize}
  \item
    ...
    behaviour which looks pronouncedly linear, albeit with a heavy constant
  ...
  \item
    ...
    in (see \cref{fig:benchtime4,fig:benchtime5}).
\end{itemize}
...
Finally, we present, in \cref{fig:benchTSPLIB}\footnote{%
  Here, {\defstyle flipflop} refers to method of alternating
...
\begin{enumerate}[(i)]
  ...
  \item computed tour cost (as quotient to optimal solution)
    as well as running times
    over a set of TSPLIB instances sized not higher than $130$
    (sample size $25$, most of them euclidean).
\end{enumerate}
...
\null\hfill%
\subfloat[TSPLIB instances: observed tour quality]{%
  \includegraphics[width=0.67\textwidth]{../plot/build/tsplib_val.pdf}
}\hfill\null\\
\null\hfill%
\subfloat[TSPLIB instances: runtime measured]{%
  \includegraphics[width=0.67\textwidth]{../plot/build/tsplib_time.pdf}
}\hfill\null
...
Having described application of dynamic programming solutions to the
traveling salesman problem and their extension from neighbourhoods to
iterative heuristics, as well as two such neighbourhoods,
we have provided, for said two neighbourhoods,
a working implementation which
%closely follows our discussion and
is general enough to be easily
extended to other neighbourhoods and efficient enough for study of new such
solutions or combinations\footnote{%
  via shared library interface or standalone executable,
  which provides possibility for choice of starting tour.
}
of related heuristics.

For the exponential neighbourhood of strongly balanced tours this
represents first practical results; in the process, we have further
confirmed the linear time conjecture for select few node size limits.
...
\begin{itemize}
  ...
  \item
    try different flowers for strongly balanced tours;
  \item
