...
\declaretheorem[style=definition,name=Definition,refname={definition,definitions}]{define}
...
\begin{define}[(exponential) neighbourhood]
  A {\defstyle neighbourhood} ({\defstyle $\mathcal{T}_n$}) is a map
  \[
...
\begin{define}[local search]
  Given a neighbourhood $F$ as well as $M \subseteq~\mathbb{R}^{n \times n}$ and $C \in M$,
...
\begin{proof}
  ...
  and $d\restriction_{\mathcal{N}_n} = \operatorname{id}\restriction_{\mathcal{N}_n}$
...
\begin{proof}
  ...
  \begin{align*}
    \begin{split}
    \operatorname{tsp}^\ast_{\left\{C\right\},T}\left(\rho\,C\right)
    & =
...
If we now choose some (any) element $\tau^\ast \in T$ and consider $T$ to be a set
of permutations of $\tau^\ast$, its action then defines a neighbourhood over $\mathcal{T}_n$:
...
\begin{define}[induced neighbourhood]
  ...
  \]
  We call $F_T$ neighbourhood {\defstyle induced by} $T$ (and $\tau^\ast$).
\end{define}
...
\begin{remark}
  If we now can solve the TSP efficiently over a fixed set $T$,
  ...
  \]
  and choosing $\tau^\ast$ to be the starting tour then yields iterated local search.
  %
