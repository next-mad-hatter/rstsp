...
\dictum[Steve Wozniak]{Never trust a computer you can't throw out a window.}

\subsection{Motivation}

\selectlanguage{russian}
Ад еюж зальы атквюе лыгимуз. Эа фалля ийжквюы
аппэльлььантюр квюо. Кюм новум зэнтынтиаэ мэдиокретатым ыт, квуй эи лебыр
граэкы атоморюм, фырре мэльиорэ вяш ед. Мальюизчыт эффикеэнди дытыррюизщэт
ад еюж, ючю экз эзшэ путант, зыд ты квюод золэт долорэж. Хаж адипижкй
конкыптам нэ.

Вяш дэбыт ютенам зигнёфэрумквюы ан, прё нэ адвыржаряюм конклььюжионэмквуэ
витюпэраторебуз, ан про натюм фиэрэнт. Йн аффэрт граэкы тамквюам шэа, вяш
трётанё чадипжкёнг ат, нэ квуй путант хонэзтатёз ентырпрытаряш. Нэ хёнк
витюпэраторебуз шэа, эю омнеж мовэт рыкючабо эож. Татион компрэхэнжам ут
зыд. Ан векж эзшэ бландит. Эним порро эи еюж.

Польский язык \foreignlanguage{polish}{(język polski, polszczyzna)} —
язык поляков, относящийся к лехитской подгруппе 
...
\par
\selectlanguage{english}
Default fight flight affinity 1234567890
{\bfseries Hello Привет Mäx} {\scshape Привет Mäx} {\em Привет Mäx} {\scshape \bfseries Yes}
\par
{\sffamily sans
{\bfseries Hello Привет Mäx} {\scshape Привет Mäx} {\em Привет Mäx} {\scshape \bfseries Yes}
}
Let
\annotation{What is this?}
\[ f(x) = \alpha \mathrm A B \gamma 1234567890; \; \Phi_k\left(\big\{ \{a_1,b_1\}, \ldots ,\{a_p,b_p\} \big\} \right) \]
...
\begin{mplibcode}
  beginfig(0)
    ...
    for i = 1 upto length a:
      ...
      drawarrow subpath (i-1,i) of a withcolor 0.4;
    endfor
  endfig;
\end{mplibcode}
\ 
...
\hfill\null
\caption{Single traversal running times (Linux 3.10.17 x86\_64 @ i5-2520M).}
\label{fig:timeGraph}
\end{figure}
